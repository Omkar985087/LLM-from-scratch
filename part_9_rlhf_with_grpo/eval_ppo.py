...
sys.path.append(str(_P(__file__).resolve().parents[1]/'part_7_reward_modeling'))
...
def score_policy(policy_ckpt: str, rm_ckpt: str, bpe_dir: str | None, n: int = 16):
    ...
    ref = PolicyWithValue(cfg.get('vocab_size', tok.vocab_size), cfg.get('block_size', tok.block_size),
                          cfg.get('n_layer', 2), cfg.get('n_head', 2), cfg.get('n_embd', 128)).to(device)
    ckpt_ref = torch.load("../part_6_supervised_fine_tuning_sft/runs/sft-demo/model_last.pt", map_location=device) # hardcoded path to SFT checkpoint
    ...
    for p in prompts:
        ...
        from part_6_supervised_fine_tuning_sft.formatters import Example, format_example
