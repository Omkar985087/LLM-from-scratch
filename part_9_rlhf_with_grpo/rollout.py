...
sys.path.append(str(_P(__file__).resolve().parents[1]/'part_4_scaling_up'))
try:
    from tokenizer_bpe import BPETokenizer
    _HAS_BPE = True
except Exception:
    _HAS_BPE = False
sys.path.append(str(_P(__file__).resolve().parents[1]/'part_3_modernizing_the_architecture'))
try:
    from tokenizer import ByteTokenizer
except Exception:
    ByteTokenizer = None

from part_6_supervised_fine_tuning_sft.formatters import Example, format_example, format_prompt_only
